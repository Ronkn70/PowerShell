...
Start-Transcript -Path $env:SystemDrive\Windows\Temp\TempFileRemoval-$LogDate.log 
...
Get-ChildItem "$env:SystemDrive\users\*\AppData\Local\Microsoft\Windows\Temporary Internet Files\*" ` 
...
Get-ChildItem "$env:SystemDrive\inetpub\logs\LogFiles\*" -Recurse -Force -ErrorAction SilentlyContinue | 
